...
Dans un contexte où les menaces informatiques se multiplient et évoluent rapidement, les centres d’opérations de sécurité (SOC) doivent être en mesure de détecter, classifier et répondre efficacement aux incidents de cybersécurité. Pour renforcer ces capacités, Microsoft a développé la base de données GUIDE, la plus vaste collection publique d’incidents réels de sécurité à ce jour. Mise en place dans le cadre du développement du Copilot for Security Guided Response (CGR), cette base vise à améliorer l’enquête, le triage et l’assainissement des incidents de sécurité.

L’analyse de cette base de données permet de dégager des tendances et des modèles susceptibles d’aider les analystes SOC à prioriser les alertes et à identifier les menaces les plus critiques. Dans ce projet, nous proposons d’explorer plusieurs hypothèses statistiques afin de mieux comprendre les facteurs associés aux niveaux de risque et aux verdicts finaux attribués aux incidents.

Les hypothèses principales testées sont les suivantes :

H1 : Le niveau de suspicion (SuspicionLevel_Suspicious) est positivement associé à la probabilité qu’un verdict soit classé comme malicieux (LastVerdict_Malicious).

H2 : Il existe une différence significative dans le niveau d’incident (IncidentGrade) selon que l’utilisation soit privée ou publique (Usage_Private vs Usage_Public).

H3 : Certaines familles de systèmes d’exploitation (OSFamily) sont plus fréquemment associées à des incidents classés comme malveillants.

H4 : La répartition géographique des incidents (pays, États, villes) influence la fréquence ou la gravité des menaces détectées.



Ces hypothèses serviront de fondement à une série d’analyses statistiques et de modélisations, avec pour objectif ultime de dégager des pistes concrètes pour optimiser les outils d’aide à la décision des analystes SOC.
...
1. Prétraitement des données :
    ...
    1.3 Nettoyage de la base des donnée ;
    
    
2.Analyse exploratoire des données :
   
    2.1 Visualisations : Diagrammes de comparaison

      * Les diagramme en barres ;
   ...
   2.3 Apprentisssage non supervisé sur un échantillon alléeatoire.

    2.3.1 Analyse par composantes principales
   
    2.3.2 Clustering : Méthode des k-means
...
5. Conclusion :
  ...
  5.1 Quelques recommandations basées sur l'étude 
...
H0 : la moyenne journalière d’incidents en  juin est la même  que pour tous les mois.

H1 : la moyenne journalière d’incidents en juin est supérieure.

$$\alpha = 0.05 $$
*Conditions d'application du ttest:*

 * les observations sont indépendantes, du fait que les jours sont indépendants
 
 * On a plus de 30 observation --> la distribution est considérée comme normale
 
 
 * homoscédasticité --> On le teste avec le test de Levene
...
monthly_avg <- daily_counts %>%
  ...
  group_by(Month) %>%
  summarise(DailyAvg = mean(Count), .groups = "drop")


ttest_data <- daily_counts %>%
  mutate(Month = month(Date, label = TRUE, abbr = FALSE),
         Period = ifelse(Month == "juin", "Juin", "Autres mois"))
ttest_data$Period <- factor(ttest_data$Period)

levene <- leveneTest(Count ~ Period, data = ttest_data)
 cat('p-value :' , levene$`Pr(>F)`[1])
 


```
 *p-value  0.05* Donc les variances ne sont pas égales. 

On va donc appliquer le test de Welch

```{r Welch-t-test}

resultat = t.test(Count ~ Period, data = ttest_data, alternative = "less")
t_result <- t.test(Count ~ Period, data = ttest_data, alternative = "greater")

print(resultat)
```

**interprétation :** Le test de Welch a révélé une différence statistiquement significative entre le nombre d'incidents signalés en Juin et les autres mois (p < 0.05). En d'autres termes, les incidents de cybersécurité ont été significativement plus fréquents pendant le mois de juin par rapport aux autres mois de la période étudiée. Cette différence pourrait indiquer une tendance saisonnière ou une concentration d'événements liés à des facteurs spécifiques à cette période.

#### 2.3 Apprentisssage non supervisé.


#### Préparation de la base initiale
...
col_names <- colnames(df_copie)
pve <- summary(pr.out)$importance[2, ]

df_pve <- data.frame(
  Composante = 1:length(col_names),
...
```{r visualisation-PCA2}

pve <- summary(pr.out)$importance[2, ] 


df_pve <- data.frame(
  Composante = 1:length(pve),
  PVE = pve,
  PVE_Cumulee = cumsum(pve)  
)

col_names_5 <- colnames(df_copie)[1:5]
df_pve_5 <- data.frame(
  Composante = 1:length(col_names_5),
  PVE = pr.out$sdev[1:5]^2 / sum(pr.out$sdev^2),  
  Colonne = col_names_5 
)

p1 <- ggplot(df_pve_5, aes(x = factor(Composante), y = PVE)) + 
  geom_bar(stat = "identity", fill = "steelblue", color = "black", width = 0.7) + 
  geom_text(aes(label = Colonne), vjust = -0.5, color = "black", size = 3) + 
  labs(title = " ", x = " ", y = "PVE") + 
  theme_minimal()

p2 <- ggplot(df_pve, aes(x = Composante, y = PVE)) +
  geom_line(color = "steelblue", size = 1) +  
  geom_point(color = "steelblue", size = 2) +  
  labs(title = "Proportion de variance expliquée", x = "Composante principale", y = "PVE") + 
  theme_minimal()


p3 <- ggplot(df_pve, aes(x = Composante, y = PVE_Cumulee)) +
  geom_line(color = "purple", size = 1) +
  geom_point(color = "purple", size = 2) +
  labs(title = "Variance expliquée cumulée", x = "Composante principale", y = "PVE cumulée") +
  theme_minimal()
...
Avec 20 composantes principales, on est capable d'expliquer environ 95% de la variabilité des observations.
...
qu'il y a trois groupes différents ayant pour tailles respectives `r km.out$size[1]` observations, `r km.out$size[2]` observations et `r km.out$size[3]` observations.
...
```{r construction-arbre,eval=FALSE,include=FALSE}
...
Lors du calcul de la matrice de confusion, nous avons utilisé par défaut un seuil de classification (cutoff) fixé à 0,5. Toutefois, ce seuil pourrait être optimisé en utilisant l'indice de Youden, qui consiste à évaluer l’ensemble des seuils possibles afin de maximiser la somme de la sensibilité et de la spécificité. Cette approche permet de déterminer le cutoff optimal offrant le meilleur équilibre entre la détection des vrais positifs et la minimisation des faux positifs, c'est à dire **minimiser à la fois les faux positifs et les faux négatifs.**

**[Youden, W. J. (1950). Index for rating diagnostic tests. Cancer, 3(1), 32–35,**

**PMID: 15405679 DOI: 10.1002/1097-0142(1950)3:1<32::aid-cncr2820030106>3.0.co;2-3]**

* implementation avec r :

**https://stackoverflow.com/questions/73144637/ho-to-make-the-receiver-operating-characteristic-roc-and-identifying-the-youde**
